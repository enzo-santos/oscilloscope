...
void main() {
  ...
  test("Global resizer should keep viewport on default", () {
    final RealTimeTraceProvider controller = RealTimeTraceProvider(
      initialViewport: Viewport(Range(0, 10), Range(-1, 1)),
      xResizer: XResizer.global(),
      yResizer: YResizer.global(),
  ...
  group("Global resizer should change viewport on overflow", () {
    RealTimeTraceProvider? controller;
    setUp(() {
    ...
    test("Horizontal underflow", () {
      controller?.add(-15, 0);
      expect(controller?.viewport, Viewport(Range(-15, 10), Range(-1, 1)));
    });
    test("Horizontal overflow", () {
      controller?.add(15, 0);
      expect(controller?.viewport, Viewport(Range(0, 15), Range(-1, 1)));
    });
    test("Vertical underflow", () {
      controller?.add(0, -5);
      expect(controller?.viewport, Viewport(Range(0, 10), Range(-5, 1)));
    });
    test("Vertical overflow", () {
      controller?.add(0, 5);
      expect(controller?.viewport, Viewport(Range(0, 10), Range(-1, 5)));
    });
    test("Total overflow", () {
      controller?.add(-15, 5);
